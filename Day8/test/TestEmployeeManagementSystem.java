	...
	public static void main(String[] args) {
		...
		do {
			...
			case 8 ->{
			    ...
			    List<Employee> elst = eservice.findBySalary(salary); // Assume this now returns an empty list if nothing is found

			    // Check if the list is empty (more robust than checking for null)
			    if (!elst.isEmpty()) {
			        System.out.println("--- Employees Found with Salary " + salary + " ---");
			        elst.forEach(System.out::println);
			    }
			    else {
			...
			case 9 -> {
				System.out.println("enter salary for search");
				 double sal = sc.nextDouble();
				List<Employee> lst=eservice.sortBySal(sal);
